...
@pytest.mark.parametrize("project_name,error_message", [
    ("", 'Required field "name" is missing'),
    (" ", 'Invalid project name specified. Project names cannot be blank.')])
def test_mantis_project_creation_negative(client, project_name, error_message):
    project = client.factory.create('ProjectData')
    project.name = project_name

    with pytest.raises(suds.WebFault) as exeption:
        project_id = client.service.mc_project_add(username, password, project)
    assert error_message in str(exeption.value)
